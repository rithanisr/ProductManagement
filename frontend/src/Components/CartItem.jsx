        ...
        <div className="col">
          <div className="d-flex justify-content-between align-items-start gap-3 mb-2">
            <div>
              ...
              <p className="text-muted small mb-1">{product.category?.name || "Unassigned"}</p>
              <p className="text-muted small mb-0">${Number(product.price || 0).toLocaleString()}</p>
            </div>
            <button type="button" className="btn btn-sm btn-outline-danger" onClick={() => onRemove(product.id)}>
              Remove
            </button>
          </div>
